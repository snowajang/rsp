...
router.get('/', ensureAuth, async (req, res, next) => {
  try {
    const [
      ...
      req.prisma.member.count({ where: { isActive: true } }),
      req.prisma.member.count({ where: { isActive: false } }),
    ...
    res.render('dashboard', {
      ...
      latestUsers: Array.isArray(latestUsers) ? latestUsers : [],
      recentApiLogs: Array.isArray(recentApiLogs) ? recentApiLogs : [],
    });
  } catch (err) {
    next(err);
  }
